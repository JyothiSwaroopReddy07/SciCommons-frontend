...
const SubmitCommunity = ({article, setShow}) => {
    ...
    const loadCommunityId = async (res) => {
        setCommunityId(res)
    }
    ...
    const verify = async(res) => {
        for(let i=0;i<res.length;i++){
            if(res[i].Community_name.toLowerCase() === community.toLowerCase()){
                await loadCommunityId(res[i].id);
                return true;
            }
        }
        return false;
    ...
    const handleCommunityName = async() => {
        ...
        try{
            const res = await axios.get(`https://scicommons-backend.onrender.com/api/community/?search=${community}`,config);
            console.log(res)
